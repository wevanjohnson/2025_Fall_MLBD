...
\center
![](treeFigs/heartAttack.png){width=50%}
...
\small
```{r polls-2008-again, out.width='45%', fig.height=3, fig.width=6, fig.align='center'}
data("polls_2008")
...
## Regression trees
If we set `cp = 0` and `minsplit = 2`, then our prediction is too flexible:
...
\footnotesize
```{r mnist-27-tree,out.width='75%', fig.height=3, fig.width=6, fig.align='center'}
...
Random forests, explained next, improve on several of these shortcomings. This problem is solved by **averaging** many unstable trees, which leads us to __bootstrap aggregation__ or __bagging__. 
